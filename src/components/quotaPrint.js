...
function QuotaPrint() {
    ...
    useEffect(() => {
        ...
        // Fetch sumYear data for PieChartYear
        axios.get('http://localhost:5000/api/sumYear')
            .then(response => {
                setSumYearData(response.data);

                // Set initial filter year to the most recent year in sumYearData
        ...
        // Fetch sumUser data for PieChartList
        axios.get('http://localhost:5000/api/sumUser')
            .then(response => {
                setSumUserData(response.data);
                // Filter sumUserData based on filter criteria
